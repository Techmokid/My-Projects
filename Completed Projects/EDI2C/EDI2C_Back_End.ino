...
//These variables are for error correction
int maxNumberOfWarnings = 5;
int currentWarningCount = 0;
unsigned long previousWarningTimer = millis();
int warningIgnoreTimer = 1000;
bool destroy = false;
...
void sendCorruptDataWarning(int address) { sendCorruptDataWarning(address,""); }
void sendCorruptDataWarning(int address, String params) {
  if (params == "") {
    sendRawMessage("CD",address);
  } else {
    sendRawMessage("CD:" + params,address);
  }
}

void handleMasterControl() {
  if (destroy && isMaster) {
    sendRawMessage("LN",my_EDI2C_Address);
    releaseMaster();
    if (!isMaster) {
      digitalWrite(masterLock,LOW);
      digitalWrite(transmittingMsg,LOW);
      digitalWrite(NetData,LOW);
      digitalWrite(NetClk,LOW);
      digitalWrite(AddrReqNew,LOW);
  ...
  if ((currentWarningCount > 0) && (micros() - previousWarningTimer > warningIgnoreTimer)) {
    previousWarningTimer = micros();
    currentWarningCount--;
  ...
  for (int i = 0; i < receivedDataBuffer.size(); i++) {
    ...
    if (addr == -1) {
      ...
      if (addr == my_EDI2C_Address) {
        ...
        if (currentWarningCount > maxNumberOfWarnings) {
          destroy = true;
        }
      }
      ...
      //Remove the network value of "addr" frrom memory
      for(int z = 0; z < allNetworkAddresses.size(); z++) {
        if (allNetworkAddresses[z] == addr) {
          allNetworkAddresses.remove(z);
        }
      }
      
      receivedDataBuffer.remove(i);
    } else {
      sendCorruptDataWarning(addr,command + ":" + commandParams);
      Serial.println("Received corrupt data from address: " + addr);
      Serial.println(" - Received Data: " + command + ":" + commandParams);
      receivedDataBuffer.remove(i);
    }
  }
  ...
  while (!digitalRead(masterLock)) {
    if (micros() - tempTime > 12000 + my_EDI2C_Address * 10) {
      digitalWrite(masterLock,HIGH);
      isMaster = true;
      sendRawMessage("MS",my_EDI2C_Address);
    }
...
void getNextUserData(int &addr, String &response) {
  for (int i = 0; i < receivedDataBuffer.size(); i++) {
    String completeCommand = receivedDataBuffer.get(i);
    int commandAddr = completeCommand.substring(0,completeCommand.indexOf('|')).toInt();
    ...
    if ((commandAddr == my_EDI2C_Address) && (command == "GD")) {
      addr = commandAddr;
      response = command;
      receivedDataBuffer.remove(i);
      break;
...
void sendUserData(String data, int address) {
  sendRawMessage("GD:" + data,address);
}

void sendUserData(int address, String data) {
  sendRawMessage("GD:" + data,address);
}

int getMyAddress() {
  return my_EDI2C_Address;
}

void sendUserDataToAllDevices(String data) {
  for (int i = 0; i < allNetworkAddresses.size(); i++) {
    if (allNetworkAddresses[i] != my_EDI2C_Address) {
      sendRawMessage("GD:" + data, allNetworkAddresses[i]);
    }
  }
}
