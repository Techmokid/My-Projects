        ...
        public static string getHistoricTrades(string symbol, int limit, long fromId)
        {
            if (!createdJSON) {
                ...
                try {
                    ...
                    Debug.LogError("Failed to save historic data cache to directory: " + filePath + "HistoricDataCache.json");
                    Debug.LogError("Please check that the file path is accessable, and this program has permissions to write to it");
                    Application.Quit();
                }
            }
