    ...
    public partial class Program
    {
			...
			public void Execute() {
				...
				switch (state) {
					...
					case 2:
						...
						break;
					case 3:
						outputImage[index] = (float)Math.Pow(CalculateLineWeights(ThreadIds.X),3);
						break;
					default:
						outputImage[index] = 0;
						break;
		...
		public readonly partial struct finalPass1 : IComputeShader
		{
			...
			public void Execute() {
				...
				//WIP W.I.P ERROR: This function creates looped pixels ( thisX=nextX or thisY=nextY )
				
				if (direction) {
					// Specifically this section for some reason throws the errors we are seeing. When direction=false, we don't get this issue
					float BL = GetPixelFloat(x-1,y+1);
					float BM = GetPixelFloat(x,  y+1);
					float BR = GetPixelFloat(x+1,y+1);
					
					int resultingDirection = 0;
					if ((BL < BM) && (BL < BR))
					...
					//Actually make the final move
					pixelAnalyzer i = GetStructAtPosition(x,y);
					if (resultingDirection == -1) {
						i.nextX = x-1; //Go to the bottom left
					} else if (resultingDirection == 1) {
						i.nextX = x+1; //Go to the bottom right
					} else {
						i.nextX = x; //Go to the bottom middle
					}
					...
					int resultingDirection = 0;
					if ((RU < RM) && (RU < RL))
						resultingDirection = -1; //RU is least costly
					if ((RM < RU) && (RM < RL))
						resultingDirection = 0; //RM is least costly
					if ((RL < RM) && (RL < RU))
						resultingDirection = 1; //RL is least costly
					if (RL == RU && RU == RM)
						resultingDirection = 0;
					if (RL == RU) {
						if (RL < RM) {
